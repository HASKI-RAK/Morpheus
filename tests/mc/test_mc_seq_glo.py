...
from source.synthesizer.LPaths.mc import mc_seq_glo
...
class UnitTests(unittest.TestCase):
    ...
    def test_model_load(self):
        ...
        """
        # get path to sequential data json file
        current_path = os.getcwd()
        path_parent = current_path.replace(r"\tests", r"\source")
        path = path_parent + r"\synthesizer\LPaths\mc\models\sequential.json"
        first_le_probabilities_expected = {
            ...
            5: 0.20408163265306123,
            6: 0.0,
            7: 0.0,
            8: 0.02040816326530612,
        }
        # load model and initial distribution matrix
        model, first_le_probabilities = mc_seq_glo.load_model(path)
    ...
    def test_dict_conversion(self):
        ...
        self.assertEqual(mc_seq_glo.convert_keys_to_int(test_dict),
                         expected_dict)
    ...
    def test_data_synthesis_amount(self):
        ...
        correctly
        """
        # get path to global data json file
        current_path = os.getcwd()
        path_parent = current_path.replace(r"\tests", r"\source")
        path = path_parent + r"\synthesizer\LPaths\mc\models\global.json"
        model, first_le_probs = mc_seq_glo.load_model(path)
        # synthesize learning paths
        synthetic_lps = mc_seq_glo.sample_learning_paths(model, first_le_probs,
                                                         50)
    ...
    def test_synthesize_data(self):
        ...
        """
        # synthesize learning paths
        synthetic_lps = mc_seq_glo.synthesize_data("Sequential",
                                                   10)
